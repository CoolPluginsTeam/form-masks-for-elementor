...
class CPFM_Feedback_Notice {
    ...
    public function cpfm_enqueue_assets() {
        ...
        wp_enqueue_style('cpfm-common-review-style', FME_PLUGIN_URL . 'admin/feedback/css/cpfm-admin-feedback.css', null, FME_VERSION);


        wp_enqueue_script(
            'cpfm-common-review-script', 
            FME_PLUGIN_URL . 'admin/feedback/js/cpfm-admin-feedback.js', 
            ['jquery'], 
            FME_VERSION,
            true
    ...
    public function cpfm_render_notice_panel() {
        ...
        $output .= '<div class="notice-panel-header">' . esc_html__('Help Improve Plugins', 'form-masks-for-elementor') . ' <span class="dashicons dashicons-no" id="cpfm_remove_notice"></span></div>';
        ...
        foreach (self::$registered_notices as $key => $notice) {
            ...
            $output .= '<p>' . esc_html($notice['message']) . '<a href="#" class="cpf-toggle-extra">' . esc_html__(' More info', 'form-masks-for-elementor') . '</a></p>';
            $output .= '</div>';
            ...
            $output .= '<button class="button button-primary opt-in-yes" data-category="' . esc_attr($key) . '" id="yes-share-data" value="yes">' . esc_html__("Yes, I Agree", 'form-masks-for-elementor') . '</button>';
            $output .= '<button class="button opt-in-no" data-category="' . esc_attr($key) . '" id="no-share-data" value="no">' . esc_html__('No, Thanks', 'form-masks-for-elementor') . '</button>';
            $output .= '</div>';
